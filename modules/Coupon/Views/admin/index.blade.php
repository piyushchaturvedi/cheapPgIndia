...
@section('content')
    <div class="container">
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                <form method="post" action="{{url('admin/module/coupon/bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                    {{csrf_field()}}
                            ...
                            @if($rows->total() > 0)
                                @foreach($rows as $row)
                                    <tr>
                                        ...
                                        <td class="title">
                                            <a href="{{url('admin/module/coupon/edit/'.$row->id)}}"> {{$row->code}}  </a>
                                        </td>
                                        <td class="author">
