...
use Modules\Hostel\Models\HotelTranslation;
...
class Hotel extends Bookable
{
    ...
    public    $type                               = 'hotel';
    public    $checkout_booking_detail_file       = 'Hostel::frontend/booking/detail';
    public    $checkout_booking_detail_modal_file = 'Hostel::frontend/booking/detail-modal';
    public    $email_new_booking_file             = 'Hostel::emails.new_booking_detail';
    protected $fillable      = [
        'title',
        'content',
        'status',
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('hostel.hostel_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public static function getLinkForPageSearch($locale = false, $param = [])
    {
        ...
        return url(app_get_locale(false, false, '/') . config('hostel.hostel_route_prefix') . "?" . http_build_query($param));
    }
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'hostel')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('hotel_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (empty($number))
            return false;
        if ($number > 1) {
            return __(":number Hostels", ['number' => $number]);
        }
        return __(":number Hostel", ['number' => $number]);
    }
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                ...
                $res[] = [
                    ...
                    'price_html'      => format_money($room->tmp_price) . '/ months',
                    'price_text'      => format_money($room->tmp_price) . '/' . 'months'
                ];
                $this->tmp_rooms[] = $room;
            }
        }
    ...
    public function detailBookingEachDate($booking){
	    $rooms = \Modules\Hostel\Models\HotelRoomBooking::getByBookingId($booking->id);
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($accomodation_for = $request->query('accomodation_for'))) {
            $model_hotel->where("bravo_hotels.accomodation_for",$request->query('accomodation_for'));
        ...
        if (!empty($price = $request->query('price'))) {

            $raw_sql_min_max = "(  bravo_hotels.price <= ? )";
            $model_hotel->WhereRaw($raw_sql_min_max,[$price]);
        }
