...
@section('content')
    <h2 class="title-bar no-border-bottom">
        {{$row->id ? __('Edit: ').$row->title : __('Add new hostel')}}
        @if($row->id)
            <div class="title-action">
                <a class="btn btn-info" href="{{route('hostel.vendor.room.index',['hotel_id'=>$row->id])}}">
                    <i class="fa fa-hand-o-right"></i> {{__("Manage Rooms")}}
                </a>
                <a href="{{route('hostel.vendor.room.availability.index',['hotel_id'=>$row->id])}}" class="btn btn-warning">
    ...
    <div class="lang-content-box">
        <form action="{{route('hostel.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
            @csrf
...
{{--                        <a data-toggle="tab" href="#nav-ical" aria-selected="false">{{__("5. Ical")}}</a>--}}
                ...
                <div class="tab-content" id="nav-tabContent">
                    ...
                    <div class="tab-pane fade" id="nav-tour-location">
                        @include('Hostel::admin/hotel/location',["is_smart_search"=>"1"])
                    </div>
                    @if(is_default_lang())
