		...
		<div class="review-section total-review">
			...
			<ul class="review-list">
				...
				@if(!empty($booking->buyer_fees))
					...
					@php } @endphp
				@endif
				@if(session('discounted_price')!=0)
                <li>
                    <div class="label">
                       Discounted Price
                    </div>
                    <div class="val">
                        <span class="discounted_span">{{session('discount')}}</span>
				...
				@endif
				@php
						$tax_amt=session('discounted_price')?session('discounted_price')*(18/100):$booking->total_before_fees*(18/100);
						@endphp
				<li>
                    <div class="label">
                      Tax (18%)
				...
				<li class="final-total d-block">
					<div class="d-flex justify-content-between">
						...
						@php
						$show_price=session('discounted_price')?session('discounted_price')*(1+18/100):$booking->total+$tax_amt;
						@endphp
						<div class="val" id="booking_total">{{format_money($show_price)  }}</div>
                        {{-- <input type="hidden" name="amount" value="{{$booking->total}}" id="amount_total"> --}}
					</div>
					...
					@foreach($rooms as $room)
						<li class="mb-3 pb-1 border-bottom">
							...
							@if(!empty($dateDetail[$room->room_id]))
								<div>
									@includeIf("Hostel::frontend.booking.detail-room",['roomDate'=>$dateDetail[$room->room_id]])
								</div>
							@endif
