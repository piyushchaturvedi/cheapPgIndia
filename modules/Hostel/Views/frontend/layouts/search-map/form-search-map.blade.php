<form action="{{url( app_get_locale(false,false,'/').config('hostel.hotel_route_prefix') )}}" class="form bravo_form d-flex justify-content-start" method="get" onsubmit="return false;">
    ...
    @if(!empty($hotel_map_search_fields))
        @foreach($hotel_map_search_fields as $field)
            @switch($field['field'])
                @case ('location')
                    @include('Hostel::frontend.layouts.search-map.fields.location')
                @break
                @case ('attr')
                    @include('Hostel::frontend.layouts.search-map.fields.attr')
                @break
                @case ('date')
                    @include('Hostel::frontend.layouts.search-map.fields.date')
                @break
                @case ('price')
                    @include('Hostel::frontend.layouts.search-map.fields.price')
                @break
                @case ('advance')
