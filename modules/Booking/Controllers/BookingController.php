...
// echo $booking->total_before_fees;die;
        $booking->tax_amount=$booking->total_before_fees * (18/100);
    ...
    public function doCheckout(Request $request)
    {
        ...
        $booking->gateway = 'payU';
        $booking->wallet_credit_used = $credit;
        $booking->wallet_total_used =0;
        $booking->pay_now = floatval($booking->deposit == null ? $booking->total : $booking->deposit);
        $booking->applied_coupon=session('applied_coupon');
        $booking->actual_price=$booking->total;
       // $booking->total=session('discounted_price')??$booking->total;
        if(session('code_id'))
        {
            $coupon=Coupon::find(session('code_id'));
            $coupon->usage_count=$coupon->usage_count + 1;
            $coupon->save();
            $booking->discount=session('discount');
        }
...
//        event(new VendorLogPayment($booking));
    ...
    {
    //     $rdata=$request->all();
    //     $rdata['start_date']=session('start_date');
    //     $rdata['end_date']=session('end_date');
	...
	public function exportIcal($service_type = 'tour', $id)
	{
		\Debugbar::disable();
		$allServices = get_bookable_services();
		$allServices['room']='Modules\Hostel\Models\HotelRoom';
		if (empty($allServices[$service_type])) {
            return $this->sendError(__('Service type not found'));
		}
		$module = $allServices[$service_type];
    ...
    public function get_copupon_detail(Request $request)
    {
        $booking = $this->booking::where('code', $request->page_code)->first();
        ...
        if(!empty($coupon))
        {
            ...
            {
                $result['discount']=(($booking->total_before_fees*$coupon->percentage)/100);
                $result['discounted_price']=$booking->total_before_fees - (($booking->total_before_fees*$coupon->percentage)/100);
                $result['status']=1;
                $result['message']='Successfully Applied.';
                session(['code_id'=>$coupon->id,'applied_coupon'=>$coupon->code,'discount' =>$result['discount'],'discounted_price'=>$result['discounted_price']]);
            }
            else
            {
                $result['status']=0;
                $result['message']='Invaid Coupon.';
            }
    ...
    {
        $booking = $this->booking::where('code', $_POST['productinfo'])->first();
        $booking->status = $booking::PAID;
        $booking->paid= $_POST['amount'];
        $booking->save();
        $user=User::where('id',$booking->customer_id)->first();
        if(!empty($user))
        {
            $msg=urlencode("Dear ".$user->name. ", your booking is confirmed. You will get an email confirmation shortly.");
            $ch = curl_init("http://bulksms.webstag.net/rest/services/sendSMS/sendGroupSms?AUTH_KEY=3cdeb6689249ecdb4fa721b2438141fd&message=$msg&senderId=DEMOOS&routeId=3&mobileNos=".$user->phone."&smsContentType=english");
    ...
    //   echo '<pre>';  print_r($booking);die;
        event(new BookingCreatedEvent($booking));
      //  $booking = $this->bookingInst;
       return redirect('booking/'.$_POST['productinfo']);
    }
}
