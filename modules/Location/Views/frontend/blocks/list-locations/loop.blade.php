@php
    ...
    */
    ...
    // $service_type[1]='hostel';

@endphp
<div class="destination-item @if(!$row->image_id) no-image  @endif">
    @if(!empty($link_location)) <a href="{{$link_location}}">  @endif
        <div class="image" @if($row->image_id) style="background: url({{$row->getImageUrl()}}); cursor:pointer;" @endif onclick="window.location='{{ $row->getLinkForPageSearch( 'hostel' ) }}';" >
